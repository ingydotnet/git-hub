...
To do a proper system install, which puts C<git-hub> next to your other git
...
If you used C<make install> method, then:
...
L<https://github.com/carloslima/git-hub-travis>
...
Upgrade the C<git-hub> installation to the latest version.
...
=item `watching L<< <user> >>:

Show which repos a user is watching.

=item `watchers L<< <owner>/<repo> >>:
...
=item C<< pull-request [<owner>/<repo>] >>
...
=item C<< url [<owner>/<repo>] [<file-path>] >>
...
=item `token-get <api-token-id> <data-key>
...
can be gleaned from the remote. Also on the C<edit> command notice how you can
...
yours) that you set with the C<git hub config login ...> command.
